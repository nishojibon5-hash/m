...
export default function MainLayout({
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col pb-24 md:pb-0">
        ...
        <div className="flex items-center justify-around h-20">
          {navItems.map(({ id, label, icon: Icon, href }) => {
            ...
            return (
              ...
              >
                {id === "upload" ? (
                  <div className="rounded-full bg-primary w-12 h-12 flex items-center justify-center mb-1">
      ...
      <nav className="hidden md:block fixed left-0 right-0 bottom-0 bg-background border-t border-border">
          ...
          {navItems.map(({ id, label, icon: Icon, href }) => {
            ...
            return (
              <Link
                key={id}
                to={href}
                className={`flex items-center gap-2 px-6 h-16 transition-colors ${
                  isActive ? "text-primary border-b-2 border-primary" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                {id === "upload" ? (
                  <div className="rounded-full bg-primary px-4 py-2 flex items-center gap-2">
